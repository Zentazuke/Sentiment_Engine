...
def signals(symbol_key: str) -> Dict[str, Any]:
    ...
    null = unavailable (never a fake zero). Best-effort per source - never raises.
    ...
    Field map (engine -> contract):
      ...
      outlook horizon (3600s) .score    -> outlook_1h
    """
    out: Dict[str, Any] = {
    ...
    try:
        ...
        pass
    try:
        # horizons is a LIST of dicts (each {horizon_seconds, score, ...}); 1h = 3600s.
        hz = (outlook(symbol_key) or {}).get("horizons") or []
        for h in hz:
            if isinstance(h, dict) and h.get("horizon_seconds") == 3600:
                out["outlook_1h"] = h.get("score")
                break
    except Exception:
        pass
    return out
